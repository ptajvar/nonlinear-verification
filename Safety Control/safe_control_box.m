function sol = safe_control_box(SpecA, SpecB, pwa, r)
...
for i = 1:nx
    P = P.Add_Constraint({'SmallestGenerator', {'G',i}},[1 -r(i)],0);
end
...
for i = 1:prodBin
    ...
    for j = 1:nu
        name = ['UG_' num2str(i) '_' num2str(j)];
        P = P.Add_Variable(name,'double',nx);
        % ulim constraint (u \in U)
        name = ['absUG_' num2str(i) '_' num2str(j)]; % absolute valu of Ac
        P = P.Add_Variable(name,'double',nx);
        for it = 1:nx
            P = P.Add_Constraint({{['absUG_' num2str(i) '_' num2str(j)],it},...
                {['UG_' num2str(i) '_' num2str(j)],it}},[-1 1],0);
            P = P.Add_Constraint({{['absUG_' num2str(i) '_' num2str(j)],it},...
                {['UG_' num2str(i) '_' num2str(j)],it}},[-1 -1],0);
        end
        P = P.Add_Constraint({{['absUG_' num2str(i) '_' num2str(j)],1:nx},...
            {['fc_' num2str(i)],j}},[ones(1,nx) 1],ulim(j,2));
        P = P.Add_Constraint({{['absUG_' num2str(i) '_' num2str(j)],1:nx},...
            {['fc_' num2str(i)],j}},[ones(1,nx) -1],-ulim(j,1));
        %end - ulim constraint
    end
    ...
    for j = 1:nx % row of A
        ...
        for jj = 1:nx % column of A
            ...
            for iu = 1:nu
                vars{end+1} = {['UG_' num2str(i) '_' num2str(iu)],jj};
    ...
    for j = 1:nx
        ...
        vars{end+1} = {'G',j};
        multipliers = [multipliers -1];
        P = P.Add_Constraint(vars,multipliers,M*2*nx-Wlim(j,2)-f(j));
        ...
        vars{end+1} = {'G',j};
        multipliers = [multipliers -1];
        P = P.Add_Constraint(vars,multipliers,M*2*nx+Wlim(j,1)+f(j));
    end
